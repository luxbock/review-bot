...
class MergeBaseTest(unittest.TestCase):
    ...
    def test_usable_recorded_base_preserves_merged_pr_diff(self):
        ...
        with checkout:
            # changed_files_block returns (block, mode, stats) since issue #21's
            # instrumentation and the empty-verdict calibration landed (the middle value
            # became a three-state DiffMode in #34) — unpack, or `assertIn` would test
            # tuple membership rather than the substring it looks like it tests.
            diff_block, mode, _stats = self.review.changed_files_block(
                checkout.wt, merge_base, self.auth
            )

        self.assertIn("pr-change.txt", diff_block)
        self.assertIn("+content changed by the PR", diff_block)
        self.assertTrue(mode.fully_inlined)
